...
SslCertificate::SslCertificate(QObject *parent) :
    QObject(parent)
{
    if (skvm::DataDirectories::profile().empty()) {
...
{
    auto cert_path = skvm::DataDirectories::ssl_certificate_path();

    if (!skvm::fs::exists(cert_path) || !is_certificate_valid(cert_path)) {
        try {
            auto cert_dir = cert_path.parent_path();
            if (!skvm::fs::exists(cert_dir)) {
                skvm::fs::create_directories(cert_dir);
            }

            skvm::generate_pem_self_signed_cert(cert_path.u8string());
...
void SslCertificate::generate_fingerprint(const skvm::fs::path& cert_path)
{
    try {
        auto local_path = skvm::DataDirectories::local_ssl_fingerprints_path();
        auto local_dir = local_path.parent_path();
        if (!skvm::fs::exists(local_dir)) {
            skvm::fs::create_directories(local_dir);
        }

        skvm::FingerprintDatabase db;
        db.add_trusted(skvm::get_pem_file_cert_fingerprint(cert_path.u8string(),
...
bool SslCertificate::is_certificate_valid(const skvm::fs::path& path)
{
    OpenSSL_add_all_algorithms();
    ERR_load_crypto_strings();

    auto fp = skvm::fopen_utf8_path(path, "r");
    if (!fp) {
        emit info(tr("Could not read from default certificate file."));
        return false;
    }
    auto file_close = skvm::finally([fp]() { std::fclose(fp); });
    ...
    auto pubkey_free = skvm::finally([pubkey]() { EVP_PKEY_free(pubkey); });
